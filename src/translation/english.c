...
static translation_string all_strings[] = {
    ...
    {TR_CONFIG_LANGUAGE_LABEL_TOOLTIP, "Allows to choose another language than the default one installed with Caesar 3, either it is a different language or a modded one.\n To use another language, create a folder (named as you wish, for example \"name of the language\") at the root of the Caesar III installation directory, then copy these 4 files inside : c3.eng, c3_map.eng, c3_map_mm.eng, andc3_mm.eng. The created folder will be available in the \"Language\" drop - down list. Select it and click \"Apply\"."},
    ...
    {TR_CONFIG_SHOW_MARKET_RANGE_TOOLTIP, "Shows how far a market lady will go to fetch goods. The range is drawn only if the \"Enable market range\" option is activated."},
    ...
    {TR_CONFIG_RETIRE_AT_60_TOOLTIP, "- By default, citizens retire at age 50 and no longer contribute to your workforce. This forces you to rely heavily on immigration when city health is good and creates an unstable workforce.\n - If you enable \"Fixed worker pool - 45 % of plebian population\", then this option has no effect."},
    ...
    {TR_CONFIG_NO_SUPPLIER_DISTRIBUTION_TOOLTIP, "Prevents market buyers from selling goods to houses while traveling to restock. This gives you finer control over the distribution of goods across the city. For example, buyers restocking wine won't supply houses along their route, helping prevent unwanted patrician evolution."},
    ...
    {TR_CONFIG_GETTING_GRANARIES_GO_OFFROAD_TOOLTIP, "- Enables a cart pusher from a granary set to \"get\" a type of food to go off road to reach a granary set to \"accept\" that type of food.\n - Removes the need for a road connection between a granary set to \"accept\" a type of food andanother granary set to \"get\" that type of food.\n - As with warehouses, they will get food from the nearest \"accepting\" granary andwill ignore other \"getting\" granaries, even if they are nearer than the \"accepting\" granary."},
    ...
    {TR_CONFIG_ALLOW_EXPORTING_FROM_GRANARIES_TOOLTIP, "Before Augustus, food imports and exports, as well as Caesar requests, only worked if food was stored in warehouses. As this may be sometimes inconvenient, Augustus now allows both importing food and sending food requests directly from granaries. However, since exporting food may make people starve, this specific new feature was added as a setting."},
    {TR_CONFIG_TOWER_SENTRIES_GO_OFFROAD, "Tower sentries don't need road access from barracks"},
    {TR_CONFIG_TOWER_SENTRIES_GO_OFFROAD_TOOLTIP, "- Enables sentries to go off road to reach their assigned tower.\n - Removes the need for a road connection between the barracks andthe tower.\n - Note that towers still require road access for staffing purposes andthat unstaffed towers still cannot receive sentries."},
    {TR_CONFIG_FARMS_DELIVER_CLOSE, "Farms and wharves deliver only to nearby granaries"},
    {TR_CONFIG_FARMS_DELIVER_CLOSE_TOOLTIP, "- Prevents farm and wharf cart pushers from traveling to the opposite end of the map to deposit their harvest or catch into an accepting granary.\n - Improves efficiency by keeping cart pushers trips short.\n - Distance is calculated \"as the crow flies\", regardless of road distance.The limit is 64 tiles."},
    ...
    {TR_CONFIG_SHOW_ROAMING_PATH_TOOLTIP, "- Colours road tiles in blue, showing paths a buildings walker roams around.\n - Can only be accessed while viewing an overlay (Fire Risk for prefects, School for children, andso on), andwhen placing or left - clicking a building.\n - Green squares show where walkers spawn, red squares where they despawn. Purple squares mean the walker spawns anddespawns on the same tile."},
    ...
    {TR_CONFIG_DRAW_WEATHER_TOOLTIP, "Augustus engine allows to simulate meteorological effects (rain, snow, sandstorm), which can be adjusted with three parameters (click on \"Apply\" button to launch a preview):\n - Overlay intensity: opacity of the effect applied to the screen\n - Speed: moving speed of the particles (raindrops, snowflakes, sand)\n - Size : size of the particles (raindrops, snowflakes, sand)"},
    ...
    {TR_USER_DIRECTORIES_WINDOW_USER_PATH_TOOLTIP, 
        "- Default (Caesar III installation path, no sub-directories)\n"
            "Use the default directory of Caesar 3 (in which c3.exe is located). Any new file will be created at the root of this folder\n"
        "- Caesar III installation path, using sub-directories\n"
            "Augustus creates automatically the required sub - folders (\'campaigns\', \'config\', \'savegames\', \'scenarios\', etc.) at the root of the Caesar 3 directory, then copies any relative files already existing (.svx, .ini, etc.) in their respective sub-folders. This option neatly sorts all types of Augustus user files.\n"
        "- Recommended (user directory path of the operating system)\n"
            "Sub-folders and user files are created in the user directory of the operating system (e.g. \"C:/Users/***/AppData/Roaming\" for Windows). This option is recommended if you don't want any new file to be created in the install directory of Caesar 3.\n"
        "- Set custom directory...\n"
            "You can choose a custom directory in which all Augustus user files will be created, sorted in their respective sub-folders.\n"
        "Notes:\n"
            "- Logs are stored as text files in the OS user directory (\"C:/Users/***/AppData/Roaming/augustus/augustus\" for Windows)"},
